// Web-only OpenAI fallback implementation (no actual OpenAI SDK)
// This prevents any OpenAI imports from being included in the web bundle
...
export const sendMessage = async (message: string, conversationHistory: Array<{role: 'user' | 'assistant', content: string}> = []): Promise<string> => {
  // Web fallback responses - no actual API calls
